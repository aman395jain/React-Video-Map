...
// })
 ...
 startmap(){
  ...
  setRoutes() {
    let self = this;
    this.startmap().then((map) => {
      var startVal = this.state.routeArray[0].location//document.getElementById("start").value
      var endVal = this.state.routeArray[this.state.routeArray.length - 1].location//document.getElementById("end").value;
      if (!startVal || !endVal) {
      ...
      for (var i = 0; i < startLoc.length; i++) {
        ...
        directionsService.route(request,
          ...
          //   }
          // });
          self.makeRouteCallback(map, i, directionsDisplay[i]), rendererOptions);
      }
    }).catch(err =>{
  ...
  makeRouteCallback(map, routeNum, disp, rendererOptions) {
    ...
    return function (response, status) {
      ...
      if (status == window.google.maps.DirectionsStatus.OK) {
        ...
        console.log("directions service successfully returns and no polylines exist already");
        ...
        for (let i = 0; i < legs.length; i++) {
          ...
          if (i == 0) {
            ...
            self.marker[routeNum] = self.createMarker(map,legs[i].start_location, "start", legs[i].start_address, "green");
          }
      ...
      if (self.polyLine[routeNum]) {
        ...
        // and start animation
        setTimeout(() => {self.startAnimation(map,routeNum)},2000);
      }
    }
  }
  ...
  updatePoly(i, d) {
    if (this.poly2[i].getPath().getLength() > 20) {
      this.poly2[i] = new window.google.maps.Polyline([this.polyLine[i].getPath().getAt(this.lastVertex - 1)]);
    }
    ...
    if (this.polyLine[i].GetIndexAtDistance(d) < this.lastVertex + 2) {
      ...
      this.poly2[i].getPath().insertAt(this.poly2[i].getPath().getLength(), this.polyLine[i].GetPointAtDistance(d));
    } else {
      this.poly2[i].getPath().insertAt(this.poly2[i].getPath().getLength(), this.endLocation[i].latlng);
    }
  }
...
console.log('computeDistanceBetween',distance);
  ...
  startAnimation(map, index) {
    ...
    this.timerHandle[index] = setTimeout(() => {self.animate(index ,50)}, 2000);  // Allow time for the initial map display
  this.index = index;
  }
  ...
  startMovement(){
    ...
    this.timerHandle[this.index] = setTimeout(() => {this.animate(this.index ,this.d)}, 2000);
  }
